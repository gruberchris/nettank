...
public class ProceduralTerrainGenerator {
    ...
    public void generateProceduralTerrain(GameMapData mapData, BaseTerrainProfile profile) {
        logger.info("Generating procedural {} terrain (seed: {}) for {}x{} map",
    ...
    private float[][] generateNoiseMap(int width, int height) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                min = Math.min(min, noiseMap[x][y]);
                max = Math.max(max, noiseMap[x][y]);
            }
        }
        
        logger.info("Noise map generated: min={}, max={}, range={}", 
                String.format("%.3f", min), String.format("%.3f", max), String.format("%.3f", max - min));
    ...
    private void assignOverlaysFromNoise(GameMapData mapData, float[][] noiseMap, BaseTerrainProfile profile) {
        ...
        logger.info("Overlay thresholds: low <= {}, high >= {} (percentiles: {}%, {}%)",
        ...
        logger.info("Assigned {} low overlays ({}) and {} high overlays ({})", 
    ...
    private void keepLargestContiguousOverlayRegion(GameMapData mapData, TerrainType targetType) {
        ...
        if (regions.isEmpty()) {
            logger.info("No overlay regions found for terrain type: {}", targetType);
            return;
        }
        ...
        logger.info("Found {} regions of type {}. Largest has {} tiles, keeping it and removing {} smaller regions",
                regions.size(), targetType, largest.size(), regions.size() - 1);
        
        // Remove all smaller regions (set overlay to null)
        ...
        for (int y = 0; y < mapData.getHeightTiles(); y++) {
            for (int x = 0; x < mapData.getWidthTiles(); x++) {
                ...
                if (!visited[x][y] && tile.hasOverlay() && tile.getOverlayType() == type) {
                    // Found unvisited tile with overlay of target type - start flood fill
    ...
    private void logTerrainDistribution(GameMapData mapData) {
        Map<String, Integer> counts = new HashMap<>();
        int total = mapData.getWidthTiles() * mapData.getHeightTiles();
        
        for (int y = 0; y < mapData.getHeightTiles(); y++) {
            for (int x = 0; x < mapData.getWidthTiles(); x++) {
                TerrainTile tile = mapData.getTile(x, y);
                String key = tile.hasOverlay() 
                    ? tile.getBaseType() + " + " + tile.getOverlayType() + " overlay"
                    : tile.getBaseType().toString();
